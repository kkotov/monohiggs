...
There are two simultaneous mechanisms of the DM production:
 * $H-S$ mixing 
 * $V^{cubic}$ potential 
...
## Mono-higgs DM production from $\small V^{cubic}$
...
## Production rate of light DM ($\tiny m_\chi < m_h/2$</span>)
...
## Heavier dark matter ($\tiny m_h/2 < m_\chi < m_S/2$)
...
* The model is not ruled out by mono-jet or other searches
* [$m_S$, $b$] is a minimal parameter set examining different event kinematics
* for $m_\chi < m_h/2$, the model is nearly agnostic to $m_\chi$
* if $m_\chi > m_h/2$ production rate is rather suppressed
